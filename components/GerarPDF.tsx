...
//     const isAnimalCastrado = atendimentos.some(at => at.castracao === true); //castração
...
//       `Castrado: ${isAnimalCastrado ? 'Sim' : 'Não'}`, //castração
...
//           { label: 'Castrado:', content: atendimento.castracao ? 'Sim' : 'Não' }
//         ];
...
const GerarPDF: React.FC<GerarPDFProps> = ({ animal, tutor, atendimentos }) => {
  ...
  const handleGeneratePDF = () => {
    ...
    // Verifica se o animal já foi castrado em algum atendimento
    const isAnimalCastrado = atendimentos.some(at => at.castracao === true);
    ...
    const animalInfo = [
      ...
      `Peso: ${animal.peso} kg`,
      `Idade: ${calcularIdade(animal.data_nascimento)}`,
      `Castrado: ${isAnimalCastrado ? 'Sim' : 'Não'}`,
      `Nascimento: ${formatDate(animal.data_nascimento)}`
    ];
    ...
    if (atendimentos.length > 0) {
      atendimentos.forEach((atendimento, index) => {
        ...
        if (yPosition > 240) { // Reduzi um pouco para garantir espaço para a info de castração
          doc.addPage();
          yPosition = 30;
        }
        ...
        const details = [
          ...
          { label: 'Conduta:', content: atendimento.tratamento },
          { label: 'Castrado:', content: atendimento.castracao ? 'Sim' : 'Não' },
          { label: 'Cirurgia:', content: atendimento.cirurgia ? 'Sim' : 'Não' }
        ];
