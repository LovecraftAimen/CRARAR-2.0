...
const StatsView: React.FC<StatsViewProps> = ({ atendimentos, animais, produtos, tutores }) => {
  ...
  const reportData = useMemo(() => {
    ...
    const totalAdotados = scopeAnimais.filter(a => Boolean(a.adotado) === true).length;
  ...
  const totalAdotadosCount = useMemo(() => animais.filter(a => Boolean(a.adotado) === true).length, [animais]);
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-6 gap-4 md:gap-6">
        {[
          { label: 'Consultas', value: filteredData.atendimentos.length, icon: Stethoscope, color: 'text-purple-600 dark:text-purple-400', bg: 'bg-purple-50 dark:bg-purple-950' },
          { label: 'Novos Pets', value: filteredData.animais.length, icon: PawPrint, color: 'text-green-600 dark:text-green-400', bg: 'bg-green-50 dark:bg-green-950' },
          { label: 'Novos Tutores', value: filteredData.tutores.length, icon: Users, color: 'text-blue-600 dark:text-blue-400', bg: 'bg-blue-50 dark:bg-blue-950' },
          { label: 'Pets Adotados', value: totalAdotadosCount, icon: CheckCircle2, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-950' },
          { label: 'Castrados', value: castradosNoPeriodo, icon: Scissors, color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-950' },
          { label: 'Pets CRARAR', value: institutionalAnimalsCount, icon: ShieldCheck, color: 'text-crarar-primary dark:text-emerald-400', bg: 'bg-crarar-primary/10 dark:bg-emerald-950' },
        ].map((stat, i) => (
          <div key={i} className="bg-white dark:bg-slate-900 p-5 md:p-6 rounded-[28px] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col justify-between space-y-4">
             ...
             <div>
                <p className={`text-[11px] font-black ${stat.color} uppercase tracking-wider leading-tight`}>{stat.label}</p>
                <p className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white mt-1.5">{stat.value}</p>
                      ...
                      <div className="bg-crarar-primary/5 p-8 rounded-[32px] border border-crarar-primary/10 relative z-10">
                         ...
                         <div className="grid grid-cols-5 gap-4">
                            <div>
                               ...
                               <p className="text-2xl font-black text-slate-900 leading-none">{reportData.stats.totalAnimais}</p>
                            </div>
                            <div>
                               <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Pets CRARAR</p>
                               <p className="text-2xl font-black text-crarar-primary leading-none">{reportData.stats.totalCrarar}</p>
                            </div>
                            <div>
                               <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Pets Adotados</p>
                               <p className="text-2xl font-black text-emerald-600 leading-none">{reportData.stats.totalAdotados}</p>
                            </div>
                            <div>
                               <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Castrações Totais</p>
                               <p className="text-2xl font-black text-blue-600 leading-none">{reportData.stats.castracoes}</p>
                            </div>
                            <div>
                               <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Responsáveis Ativos</p>
                               <p className="text-2xl font-black text-slate-900 leading-none">{reportData.stats.totalTutores}</p>
                            </div>
                         </div>
                      </div>
