...
const TutorForm: React.FC<TutorFormProps> = ({ onSave }) => {
  ...
  return (
    <div className="mx-auto w-full max-w-2xl rounded-3xl bg-white dark:bg-slate-900 p-5 md:p-8 shadow-sm border border-gray-100 dark:border-slate-800 relative">
      <div className="mb-6 flex items-center gap-3">
        <div className="rounded-xl bg-crarar-primary/10 p-2">
          <User className="h-6 w-6 text-crarar-primary" />
        </div>
        <div>
          <h3 className="text-lg md:text-xl font-bold text-slate-900 dark:text-white">Novo Tutor</h3>
          <p className="text-xs text-slate-400 dark:text-slate-400 font-medium">Preencha os dados cadastrais do responsável</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
        <div className="grid grid-cols-1 gap-4 md:gap-6 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <label className="block text-xs font-bold text-slate-500 dark:text-slate-300 uppercase mb-1">Nome Completo *</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 py-3 pl-10 pr-4 text-sm text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-crarar-primary outline-none transition-all"
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 py-3 pl-10 pr-4 text-sm font-mono text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-crarar-primary outline-none transition-all"
          ...
          <div>
            <label className="block text-xs font-bold text-slate-500 dark:text-slate-300 uppercase mb-1">Telefone / WhatsApp *</label>
            <div className="relative">
          ...
          {/* Email */}
          <div className="sm:col-span-2">
            <label className="block text-xs font-bold text-slate-500 dark:text-slate-300 uppercase mb-1">Email</label>
            <div className="relative">
      ...
      {modalStatus.type && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/70 backdrop-blur-sm p-4 animate-fade-in">
          <div className="w-full max-w-sm rounded-[32px] bg-white dark:bg-slate-900 p-8 shadow-2xl animate-slide-up text-center border border-slate-100 dark:border-slate-800">
            <div className={`mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full shadow-inner ${
              modalStatus.type === 'success' ? 'bg-green-50 dark:bg-green-900/30 text-green-500' : 'bg-red-50 dark:bg-red-900/30 text-red-500'
            }`}>
              {modalStatus.type === 'success' ? <CheckCircle2 className="h-10 w-10" /> : <AlertTriangle className="h-10 w-10" />}
            </div>
            <h4 className={`text-xl font-black mb-2 ${modalStatus.type === 'success' ? 'text-slate-900 dark:text-white' : 'text-red-600 dark:text-red-400'}`}>
              {modalStatus.type === 'success' ? 'Sucesso!' : 'Ocorreu um erro'}
            </h4>
            <p className="text-sm font-medium text-slate-500 dark:text-slate-300 mb-8 leading-relaxed">
              {modalStatus.message}
            </p>
            <button 
